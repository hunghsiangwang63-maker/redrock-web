    ...
    <div style={{ minHeight:'100vh', background:'#F7F3F3', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ background:'#fff', borderRadius:16, border:'0.5px solid #E8D5D5', padding:32, width:'100%', maxWidth:400 }}>
        <div style={{ textAlign:'center', marginBottom:24 }}>
          ...
          <div style={{ fontSize:18, fontWeight:600 }}>{t('忘記密碼')}</div>
        </div>
        {sent ? (
          <div style={{ textAlign:'center' }}>
            <div style={{ fontSize:40, marginBottom:16 }}>📧</div>
            <div style={{ fontSize:14, color:'#2D7D46', fontWeight:500, marginBottom:8 }}>{t('重設連結已寄出')}</div>
            <div style={{ fontSize:13, color:'#666', marginBottom:24, textAlign:'left' }}>{t('請查看 ')}{email}{t(' 的信箱，點擊連結重設密碼（連結 1 小時內有效）')}</div>
          ...
          <form onSubmit={handleSubmit}>
            <label style={{ fontSize:12, color:'#666', display:'block', marginBottom:5 }}>{t('註冊時填寫的 Email')}</label>
