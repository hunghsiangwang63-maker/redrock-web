...
export default function MemberInquiriesPage() {
  ...
  const GYM_OPTIONS = [{ id: 'gym-hsinchu', label: t('新竹館') }, { id: 'gym-shilin', label: t('士林館') }];
  ...
  const submit = async () => {
    if (!gymId) { setFormMsg({ ok:false, text:t('請選擇相關場館') }); return; }
    if (!subject.trim() || !content.trim()) { setFormMsg({ ok:false, text:t('請填寫標題與內容') }); return; }
    ...
    try {
      ...
      setFormMsg(null);
      loadMine();
    } catch (e) { setFormMsg({ ok:false, text: e.response?.data?.message || t('送出失敗，請稍後再試') }); }
    finally { setSubmitting(false); }
  };

  const statusBadge = (item) => item.status === 'replied'
    ? <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:10, background: item.unread ? '#FCEBEB' : '#E6F4EB', color: item.unread ? '#A32D2D' : '#2D7D46' }}>{item.unread ? t('🔴 有新回覆') : t('✓ 已回覆')}</span>
    : <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:10, background:'#F0EDED', color:'#999' }}>{t('待回覆')}</span>;
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F7F3F3', paddingBottom:80 }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#8B1A1A,#6B1414)', padding:'18px 16px 16px', color:'#fff' }}>
        ...
        <div style={{ fontSize:19, fontWeight:700 }}>{t('❓ 問題諮詢')}</div>
        <div style={{ fontSize:11, opacity:.8, marginTop:3 }}>{t('常見問題快速查詢，或直接提出您的問題')}</div>
      </div>

      <div style={{ display:'flex', background:'#fff', borderBottom:'0.5px solid #E8D5D5' }}>
        {[{ key:'faq', label:t('常見問題') }, { key:'mine', label:t('我的提問') }].map(tItem => (
      ...
      {tab === 'faq' && (
        <div style={{ padding:'12px 16px 0' }}>
          {faqLoading ? (
            ...
            <div style={{ color:'#999', fontSize:13, padding:24, textAlign:'center', background:'#fff', borderRadius:12, border:'0.5px solid #E8D5D5' }}>{t('目前尚無常見問題')}</div>
          ) : (
          ...
          <div style={{ fontSize:11, color:'#999', marginTop:16, marginBottom:8, textAlign:'left' }}>{t('沒有找到您想問的問題？')}</div>
        ...
        <div style={{ padding:'12px 16px 0' }}>
          ...
          {showForm && (
            <div style={{ background:'#fff', borderRadius:12, border:'0.5px solid #E8D5D5', padding:14, marginBottom:12 }}>
              <div style={{ fontSize:13, fontWeight:700, color:'#333', marginBottom:10, textAlign:'left' }}>{t('提出問題')}</div>
              <div style={{ marginBottom:10 }}>
                <label style={{ fontSize:11, color:'#666', display:'block', marginBottom:4, textAlign:'left' }}>{t('相關場館')}</label>
              ...
              <div style={{ marginBottom:10 }}>
                <label style={{ fontSize:11, color:'#666', display:'block', marginBottom:4, textAlign:'left' }}>{t('標題')}</label>
                <input value={subject} maxLength={60} onChange={e => setSubject(e.target.value)}
                  placeholder={t('簡短描述您的問題')}
              ...
              <div style={{ marginBottom:10 }}>
                <label style={{ fontSize:11, color:'#666', display:'block', marginBottom:4, textAlign:'left' }}>{t('內容')}</label>
                <textarea value={content} maxLength={1000} onChange={e => setContent(e.target.value)}
                  placeholder={t('請詳細描述您的問題，館方人員會盡快回覆')}
                  rows={5}
              ...
              <div style={{ display:'flex', gap:8 }}>
                <button onClick={() => { setShowForm(false); setFormMsg(null); }}
                  style={{ flex:1, height:42, borderRadius:9, border:'0.5px solid #E8D5D5', background:'#fff', color:'#444', fontSize:13, cursor:'pointer' }}>{t('取消')}</button>
          ...
          {mineLoading ? (
            ...
            <div style={{ color:'#999', fontSize:13, padding:24, textAlign:'center', background:'#fff', borderRadius:12, border:'0.5px solid #E8D5D5' }}>{t('尚未提出過問題')}</div>
          ) : (
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {mine.map(item => {
                      ...
                      <div style={{ padding:'0 14px 14px', borderTop:'0.5px solid #F0EDED', paddingTop:10 }}>
                        ...
                        {item.reply && (
                          <div style={{ marginTop:10, background:'#F7F3F3', borderRadius:8, padding:'10px 12px' }}>
                            <div style={{ fontSize:11, fontWeight:700, color:'#8B1A1A', marginBottom:4, textAlign:'left' }}>{t('館方回覆')}</div>
