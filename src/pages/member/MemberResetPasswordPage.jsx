...
export default function MemberResetPasswordPage() {
  ...
  useEffect(() => {
    if (!token) { setError(t('連結無效')); }
  }, [token]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (password !== confirm) { setError(t('兩次密碼不一致')); return; }
    if (password.length < 8) { setError(t('密碼至少 8 碼')); return; }
    setLoading(true); setError('');
    try {
      await client.post('/auth/member/reset-password', { token, newPassword: password });
      setDone(true);
    } catch(err) {
      setError(err.response?.data?.message || t('重設失敗，連結可能已過期'));
    ...
    <div style={{ minHeight:'100vh', background:'#F7F3F3', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ background:'#fff', borderRadius:16, border:'0.5px solid #E8D5D5', padding:32, width:'100%', maxWidth:400 }}>
        <div style={{ textAlign:'center', marginBottom:24 }}>
          <div style={{ fontFamily:'Georgia,serif', fontStyle:'italic', fontWeight:700, fontSize:24, color:'#8B1A1A', marginBottom:4 }}>{t('RedRock 紅石攀岩館')}</div>
          <div style={{ fontSize:18, fontWeight:600 }}>{t('設定新密碼')}</div>
        </div>
        {done ? (
          <div style={{ textAlign:'center' }}>
            <div style={{ fontSize:40, marginBottom:16 }}>✅</div>
            <div style={{ fontSize:14, color:'#2D7D46', fontWeight:500, marginBottom:8 }}>{t('密碼已成功重設')}</div>
            <div style={{ fontSize:13, color:'#666', marginBottom:24 }}>{t('請用新密碼登入')}</div>
          ...
          <form onSubmit={handleSubmit}>
            <label style={{ fontSize:12, color:'#666', display:'block', marginBottom:5 }}>{t('新密碼（至少 8 碼）')}</label>
            <PasswordInput value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" required style={inp}/>
            <label style={{ fontSize:12, color:'#666', display:'block', marginBottom:5 }}>{t('確認新密碼')}</label>
