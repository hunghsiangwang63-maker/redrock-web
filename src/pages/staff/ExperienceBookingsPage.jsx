    ...
    <div style={{ padding:24, maxWidth:900, margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={() => { setShowLinkModal(true); generateQrDataUrl(PUBLIC_BOOK_URL, { width: 240, margin: 2 }).then(setLinkQr).catch((e)=>{ reportClientError(e, 'ExperienceBookingsPage.publicLinkQr'); setLinkQr(''); }); }}
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                  <div style={{ gridColumn:'1/-1' }}>
                    <label style={{ fontSize:11, color:'#666' }}>
                      姓名 * {editBooking.needsInsurance!==false && ctNeedsInsurance(editBooking.courseType) && <span style={{ color:'#A32D2D', fontWeight:600 }}>（投保用，須填寫真實姓名，不可用暱稱代替）</span>}
                    </label>
                    <input value={p.name||''} onChange={e=>updPart(i,'name',e.target.value)} style={tinp}/>
                  </div>
