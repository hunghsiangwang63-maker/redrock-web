...
// 公開單堂工作坊報名（免登入、訪客，先轉帳）。連結格式：/book/workshop?course=<courseId>&session=<sessionId>
export default function PublicWorkshopEnrollPage() {
  ...
  useEffect(() => {
    ...
    axios.get(`${BASE}/courses/public/${courseId}`)
      .then(r => {
        ...
        if (!s) { setLoadErr('找不到此場次，可能已額滿或已結束'); return; }
        setSession(s);
      })
  ...
  const bank = session ? bankAccounts[session.gymId] : null;
    ...
    <div style={{ background: '#FBF7F7', minHeight: '100vh' }}>
      ...
      <div style={wrap}>
        <div style={card}>
          ...
          {course.description && <div style={{ marginTop: 6, fontSize: 13, color: '#666', whiteSpace: 'pre-wrap' }}>{course.description}</div>}
        ...
        <div style={card}>
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 10 }}>本人簽名（肖像權/請假退費規則同意）*</div>
