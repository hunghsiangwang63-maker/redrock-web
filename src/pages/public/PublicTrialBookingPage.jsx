...
export default function PublicTrialBookingPage() {
  ...
  useEffect(() => {
    if (!sessionId) { setLoadErr(t('連結缺少場次資訊，請聯繫櫃檯')); return; }
    publicClient.get(`/courses/public/session/${sessionId}`)
      .then(r => setInfo(r.data))
      .catch(() => setLoadErr(t('找不到此試上場次，可能已被移除或連結錯誤')));
  ...
  if (!info) return <div style={{ ...wrap, paddingTop: 60, textAlign: 'center', color: '#999' }}>{t('載入中…')}</div>;
    ...
    return (
      <div style={{ background: '#FBF7F7', minHeight: '100vh' }}>
        <div style={{ ...wrap, paddingTop: 60, textAlign: 'center' }}>
          ...
          <h2 style={{ color: RED, marginTop: 12 }}>{done.isWaitlist ? t('已加入候補名單！') : t('試上預約已送出！')}</h2>
          <div style={{ ...card, textAlign: 'left', lineHeight: 1.8 }}>
            <div>{t('感謝您預約紅石攀岩試上課程「')}{info.session.courseName}{t('」。')}</div>
            <div style={{ marginTop: 8 }}>{t('應繳金額：')}<b style={{ color: RED }}>NT${done.totalFee}</b></div>
            <div style={{ marginTop: 8, color: '#666', fontSize: 14 }}>
              {done.isWaitlist ? t('此場次已額滿，已為您排入候補，有名額釋出將依序轉正並通知您。') : t('請於期限內完成匯款，館方確認收款後即完成報名。')}
            </div>
  ...
  if (!info.allowTrial) {
    return <div style={{ ...wrap, paddingTop: 60, textAlign: 'center', color: '#A32D2D' }}>{t('此課程目前未開放試上，請聯繫櫃檯。')}</div>;
  }
  ...
  return (
    <div style={{ background: '#FBF7F7', minHeight: '100vh' }}>
      <div style={{ background: RED, color: '#fff', padding: '22px 16px', textAlign: 'center', position: 'relative' }}>
        ...
        <div style={{ fontSize: 20, fontWeight: 800, letterSpacing: 1 }}>{t('紅石攀岩 · 課程試上預約')}</div>
      ...
      <div style={wrap}>
        <div style={card}>
          ...
          <div style={{ marginTop: 10, background: '#FBF5F5', borderRadius: 10, padding: 12, fontSize: 14 }}>
            {t('試上費：')}<b style={{ color: RED, fontSize: 17 }}>NT${info.trialPrice}</b>
          </div>
        </div>

        <div style={card}>
          <div style={{ fontWeight: 700, fontSize: 15 }}>{t('報名資訊')}</div>
        ...
        <div style={card}>
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 10 }}>{t('本人簽名（免責同意）*')}</div>
        ...
        {isMinor && (
          <div style={card}>
            ...
            <div style={{ fontSize: 12, color: '#854F0B', marginBottom: 10 }}>{t('報名對象未滿 18 歲，需法定代理人一併簽名')}</div>
