...
export function InvoiceButtonAuto({ sourceType, refId, refreshToken, onClick, style }) {
  ...
  useEffect(() => {
    if (!sourceType || !refId) { setInvoiceNo(null); return; }
    let alive = true;
    client.get('/invoices/status', { params: { sourceType, refId } })
      .then(r => { if (alive) setInvoiceNo(r.data.invoiceNo || null); })
      .catch(() => { if (alive) setInvoiceNo(null); });
    return () => { alive = false; };
  }, [sourceType, refId, refreshToken]);
  return <InvoiceButtonView invoiceNo={invoiceNo} onClick={onClick} style={style} />;
}
